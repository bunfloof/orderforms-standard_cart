{*
    Cart review section, merged onto the checkout page.
    Markup sourced from this order form's own viewcart.tpl so the live cart
    (items, configurable options, quantities, remove, Apply Promo Code, tax
    estimate) is shown on the checkout page. Every variable it needs
    ($products, $addons, $domains, $promotioncode, $showqtyoptions,
    $bundlewarnings, $countries, ...) is already provided by WHMCS on the
    checkout step, so no hook is required.

    Note: the quantity / remove / promo / empty controls submit to
    cart.php (a=view / a=remove / a=empty), exactly as on the standalone
    View Cart page. standard_cart is not AJAX, so using them reloads the
    View Cart page; the customer then returns to checkout. Pure review +
    checkout (no edits) stays on this single page.
*}
<div class="cart-review-merged">
